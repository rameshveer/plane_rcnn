    ...
    for name, detector in detectors:
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            if sampleIndex < 30 or options.debug or options.dataset != '':
                # file name from source is extracted and give for plane output image.
                file_name = image_list[sampleIndex]
                visualizeBatchPair(file_name, options, config, input_pair, detection_pair, indexOffset=sampleIndex % 4000, suffix='_' + name + options.modelType, write_ply=options.testingIndex >= 0, write_new_view=options.testingIndex >= 0 and 'occlusion' in options.suffix)
                pass
            if sampleIndex >= options.numTestingImages:
                break
            continue
